...
namespace SSI.MixedReality.Toolkit.Nreal.Input
{
	...
	public class NRealDevkitController : BaseController
	{
		/// <summary>
		...
		protected void UpdateInteractions()
		{
			...
			if (TrackingState == TrackingState.Tracked)
			{
                ...
                if (!IsTouching && IsNowTouching) {
                    ...
                    Debug.Log("^^^^^^^^^^^^^^^^^^ TOUCH START" + NRInput.GetTouch());
                }
					...
					Debug.Log("VVVVVVVVVVVVVVVVVV TOUCH END");
                }
				else if (IsTouching) {
					...
					Debug.Log("TOUCH UPDATE = " + NRInput.GetTouch());
				}
			}
